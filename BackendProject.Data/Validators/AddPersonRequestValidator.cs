...
namespace BackendProject.Data.Validators
{
  public  class AddPersonRequestValidator:AbstractValidator<AddPersonRequest>
    {
        public AddPersonRequestValidator(IPersonRepository personRepository,IGenderRepository genderRepository,ITeamRepository teamRepository)
        {
            RuleFor(x => x.FirstName).MinimumLength(3).WithMessage("3 karakterden fazla giriniz").MaximumLength(15);
            RuleFor(x => x.LastName).MinimumLength(3).MaximumLength(15).NotEmpty();
            RuleFor(x => x.DateOfBirth).NotEmpty();
            RuleFor(x => x.Email).NotEmpty().EmailAddress();
            RuleFor(x => x.Mobile).MinimumLength(10).MaximumLength(10);
